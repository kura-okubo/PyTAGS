...
if int(strainmode) == 0:
    ...
    if int(temporalmode) == 0:
        ...
        for i in range (0, tri_length):
            ...
            fstrain.write('#'+'Year'+' '+'DOY'+' '+'Decimal time'+' '+'E1'+' '+'E2'+' '+'Een'+' '+'w'+' '+'theta'+' '+'Eee'+' '+'Enn'+' '+'theta_SHmax'+' '+'e_SHmax'+'\n')
            fstrain.write('##########################################################################'+'\n')
            ...
            for j in range (0, ndays):
                if (math.isnan(N1[j,i]) == False and math.isnan(N2[j,i]) == False and math.isnan(N3[j,i]) == False):
                    ...
                    theta_SHmaxstr = "{0:.2f}".format(float(L[7]))
                    e_SHmax = "{0:.4e}".format(float(L[8]))

                    fstrain.write(yearstr+' '+daystr+' '+timestr+' '+e1str+' '+e2str+' '+eenstr+' '+wstr+' '+thetastr+' '+Eeestr+' '+Ennstr+' '+theta_SHmaxstr+' '+e_SHmax+'\n')
            fstrain.close()
